...
describe("Authoritative Pricing Engine", () => {
  it("calculates accurate order totals using authoritative catalog prices", async () => {
    ...
    expect(result.total).toBe(2440 + STANDARD_SHIPPING_FEE);
    expect(result.amountInPaise).toBe((2440 + STANDARD_SHIPPING_FEE) * 100);
  ...
  it("SILENTLY IGNORES modified client-side prices and uses server prices", async () => {
    ...
    expect(result.total).toBe(2900 + STANDARD_SHIPPING_FEE);
    expect(result.amountInPaise).toBe((2900 + STANDARD_SHIPPING_FEE) * 100);
  ...
  it("calculates correct pricing for luxury gift hampers", async () => {
    const hamperCart = [
      { id: "roseate-grace", qty: 1, price: 10 }, // Real price 2030
      { id: "amethyst-elegance", qty: 1, price: 20 }, // Real price 2525
    ];

    const result = await calculateAuthoritativeOrderPricing(hamperCart);

    expect(result.subtotal).toBe(2030 + 2525);
    expect(result.total).toBe(2030 + 2525 + STANDARD_SHIPPING_FEE);
    expect(result.amountInPaise).toBe((2030 + 2525 + STANDARD_SHIPPING_FEE) * 100);
  });
