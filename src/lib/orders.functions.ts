...
export const listMyOrders = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("orders")
      .select(
        "id, order_number, status, payment_status, currency, subtotal_amount, shipping_amount, total_amount, created_at, shipping_city, shipping_state, order_items(id, product_id, name, origin, image_url, unit_amount, quantity, line_total)",
      )
